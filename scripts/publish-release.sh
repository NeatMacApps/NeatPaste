...
readonly APP_PATH="${BUILD_DIR}/DerivedData/Build/Products/Release/${APP_NAME}.app"
  ...
  if [[ -z "${submission_id}" ]]; then
    ...
    for _ in 1 2 3 4 5; do
      ...
      submission_id="$(jq -r --arg name "${basename_file}" --arg since "${started_at}" \
        '[.history[]? | select(.name==$name and .createdDate>=$s)] | sort_by(.createdDate) | last | .id // empty' \
...
readonly update_zip_path="${BUILD_DIR}/${APP_NAME}-${version}.zip"
...
codesign --force --options runtime --timestamp \
  ...
  || die "重签 ${APP_NAME}.app 失败"
sparkle_public_key="$(/usr/libexec/PlistBuddy -c 'Print :SUPublicEDKey' NeatPaste/Info.plist 2>/dev/null || true)"
...
notarize_and_wait "${BUILD_DIR}/${APP_NAME}-notarize.zip"
...
ditto "${update_zip_path}" "${appcast_dir}/${APP_NAME}-${version}.zip"
...
diskutil image create from "${stage_dir}" "${dmg_path}" \
  --format UDZO --volumeName "${APP_NAME} ${version}" >/dev/null 2>&1 \
...
if ! git diff --cached --quiet; then
  git -c core.hooksPath=/dev/null commit -m "chore: 发布 ${tag}" >/dev/null
else
  echo "工作区无改动，跳过提交"
fi
...
git add appcast.xml
if ! git diff --cached --quiet; then
  git -c core.hooksPath=/dev/null commit -m "chore: 发布 ${tag} 更新清单" >/dev/null
  git push origin main
fi
