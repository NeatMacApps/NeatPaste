...
右键菜单已有开机自启动、检查更新、隐藏菜单栏图标；设置与独立恢复窗口提供显示菜单栏图标。历史浮层点外面即关，**不能**当恢复面，也不要改成常驻。开机自启等待批准不能显示成已开启。公开依赖只写 `https://github.com/x0c/mac-kit-swift`（from 0.1.0），禁止内网地址。

本机是 Linux，没有 Xcode / Swift，**不能编译，不得宣称已验证**。回 Mac 后先 `xcodegen generate`（不要手改 xcodeproj），再按下方命令构建安装，并做：

- 图标可见时：右键有开机自启动、隐藏菜单栏图标、检查更新；左键仍只开关历史面板。
- 隐藏图标：应用不退出、剪贴板仍收录；**点隐藏时不弹窗**，只记下状态。再次打开应用或点「打开主窗口」才出现带标题栏的恢复窗口（应用仍在运行、开机自启动、检查更新、显示菜单栏图标）。关掉恢复窗口后应用仍在。
